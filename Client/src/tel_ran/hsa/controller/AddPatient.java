...
public class AddPatient extends HospitalItem {
	...
	public void perform() {
		...
		listHealthGroups.entrySet().forEach(
				pair -> inputOutput.put(String.format("%d.%s", pair.getKey(), pair.getValue().getGroupName())));
		Integer groupId = inputOutput.getInteger("Enter patient group number");
		// String[] keys = (String[])
		// HealthGroupCollection.getHealthgroups().keySet().toArray();
		...
		if (res == RestResponseCode.OK) {
			hospital.setHealthGroup(patientId, groupId);
