...
import tel_ran.hsa.protocols.api.TcpProtocol;
...
public class HsaProtocol implements Protocol {
	...
	public String getResponse(String request) {
		String []tokens=request.split(TcpProtocol.DELIMETER);
		TcpRequest type;
		try {
	...
	(Function< T, String> method,Iterable<T> iterable) {
		...
		try {
			response = StreamSupport.stream(iterable.spliterator(), false)
					.map(method)
					.collect(Collectors.joining(HsaProtocolConstantsNotUse.LINE_DELIMETER));
	...
	private String getResponseWithCode(TcpResponseCode code, String response) {
		return code.toString()+HsaProtocolConstantsNotUse.LINE_DELIMETER+response;
	}
